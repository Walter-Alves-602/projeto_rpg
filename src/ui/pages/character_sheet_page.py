...
def character_sheet_page(app, page):
    ...
    return ft.Column(
        [
            ft.Text(
                f"Ficha de Personagem: {char.nome}", size=24, weight=ft.FontWeight.BOLD
            ),
            ft.Divider(),
            # nome e informaçoes
            ft.Row(
                    ...
                    ft.Column(
                        [
                            ...
                            ft.Row(
                                [
                                    ft.Text(
                                        f"PV: {char.pontos_de_vida_atual}/{char.pontos_de_vida_max}"
                                    ),
                                    ft.ElevatedButton(
                                        "Tomar Dano",
                                        on_click=take_damage_or_heal,
                                        data="5",
                                    ),
                                    ft.ElevatedButton(
                                        "Curar", on_click=take_damage_or_heal, data="5"
                                    ),
                                ]
                            ),
                        ]
                    ),
                    ft.Column([*atributos_display]),
                ],
